...
_config();
const { API_PORT, DB_URI, MODE, COLLECTION_NAME, INDEX_NAME, OLLAMA_BASE_URL, ALLOWED_CLIENT_ORIGIN, JWT_SECRET, ADMIN_USER, ADMIN_PASSWORD, } = process.env;
...
export const apiConfig = {
    ...
    llm: {
        ...
        generate: {
            model: "qwen2.5:1.5b",
            maxTokens: 500,
            systemPrompt: "You are a helpful assistant that creates concise, descriptive titles for conversations. Given a user's question or initial message, generate a short title (3-8 words) that captures the essence of the topic. Return only the title, nothing else.",
            temperature: 0.7,
            maxRetries: 3,
        },
        embed: {
